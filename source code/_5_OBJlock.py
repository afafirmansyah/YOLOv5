...
serial_port_com2 = 'COM1'  # Ganti dengan nama port serial yang sesuai untuk COM2
...
serial_port_com3 = 'COM5'  # Ganti dengan nama port serial yang sesuai untuk COM3
...
model_weights = model_path / 'face_detections.pt'
model = torch.hub.load(model_path, 'custom', path=model_weights, source='local')
model.eval()

# Inisialisasi webcam dengan resolusi HD (1280x720)
cap = cv2.VideoCapture(1)  # Gunakan 0 untuk kamera default, atau ganti dengan alamat IP jika menggunakan IP camera
...
while True:
    ...
    if len(faces) > 0:
        # Mengambil indeks bounding box dengan area terbesar
        area = (faces[:, 2] - faces[:, 0]) * (faces[:, 3] - faces[:, 1])
        largest_face_index = torch.argmax(area)

        # Menggunakan bounding box dengan area terbesar
        faces = faces[largest_face_index].unsqueeze(0)
